...
// Доп-устройство за рубли считает ТОЛЬКО веб-чекаут (web-pay принимает device_limit); в боте
// подписка всегда на одно устройство, а лишние докупаются за токены. Поэтому обе константы
// работают лишь на пейвол веб-аккаунтов — см. paywall.dart.
...
const faqs = [
  // Раньше здесь стояло «до 10 устройств по одной подписке» — этого товара не существует:
  // подписка продаётся строго на одно устройство (PLAN_DEVICES=1), а лишние докупаются в боте
  // за токены pro-rata. Человек читал FAQ, шёл в бота за рублёвой докупкой и писал в поддержку.
